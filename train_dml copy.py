...
for epoch in tqdm(range(1, args.epoch+1),desc="Training >>>"):
    ...
    val_acc_top_1 = test_dml(ref_dataloader,validation_dataloader, model,accuracy_calculator,improveEmbedding=False,colorFeat=None)
    acc_top_1.append(val_acc_top_1)

    if prev_valid_acc<val_acc_top_1:
        # save_checkpoint(model, scheduler, optimizer, epoch, model_name, train_loss)
        print("model saved..!!")
        prev_valid_acc = val_acc_top_1
        counter = 0
    else:
        counter +=1
    if(counter==5):
